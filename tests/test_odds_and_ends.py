...
def test_liquidatePosition(
    ...
    staking.unstake(to_send, sender=strategy)
    token.transfer(gov, to_send, sender=strategy)
    ...
    vault.withdraw(vault.balanceOf(whale), whale, 10_000, sender=whale)
...
def test_rekt(
    ...
    if old_vault:
        token.transfer(strategy, 5, sender=whale)
    ...
    print(
        "Raw loss:",
        (starting_whale - token.balanceOf(whale)) / 10**18,
        "Percentage:",
        (starting_whale - token.balanceOf(whale)) / starting_whale,
    )
    print("Share price:", vault.pricePerShare() / 10**18)
...
def test_weird_reverts(
    ...
    with ape.reverts():
        strategy.migrate(whale, sender=gov)

    # can't migrate to a different vault
    with ape.reverts():
        vault.migrateStrategy(strategy, other_strategy, sender=gov)

    # can't withdraw from a non-vault address
    with ape.reverts():
        strategy.withdraw(10**18, sender=gov)
...
def test_empty_strat(
    ...
    if old_vault:
        dust_donation = 5
        token.transfer(strategy, dust_donation, sender=whale)
