...
def test_withdraw_after_donation_1(
    ...
    vault.updateStrategyDebtRatio(strategy, int(starting_debt_ratio / 2), sender=gov)
    assert vault.strategies(strategy)["debtRatio"] == int(starting_debt_ratio / 2)
    ...
    token.transfer(strategy, donation, sender=profit_whale)
    ...
    to_withdraw = int(donation / 2)
    if vault_address == ZERO_ADDRESS:
        vault.withdraw(to_withdraw, sender=whale)
    else:
        # convert since our PPS isn't 1 (live vault!)
        withdrawal_in_shares = to_withdraw * 10**18 / vault.pricePerShare()
        vault.withdraw(withdrawal_in_shares, sender=whale)
