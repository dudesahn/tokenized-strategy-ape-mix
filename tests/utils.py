...
def harvest_strategy(
    ...
    lusd_whale = accounts["0x99C9fc46f92E8a1c0deC1b1747d010903E884bE1"]
    ...
    if use_yswaps and strategy.stakedBalance() > 0:
        ...
        lusd.transfer(strategy, 200 * 10**18, sender=lusd_whale)
        lusd_whale.transfer(strategy, int(5e17))
    ...
    if strategy.emergencyExit():
        weth = project.IWETH.at(strategy.weth())
        to_deposit = strategy.balance
        weth.deposit(sender=strategy, value=to_deposit)

    # we can use the tx for debugging if needed
    tx = strategy.harvest(sender=gov)
    profit = tx.events.filter(strategy.Harvested)[0].profit / (10 ** token.decimals())
    loss = tx.events.filter(strategy.Harvested)[0].loss / (10 ** token.decimals())
...
def check_status(
    ...
    token = project.IERC20.at(vault.token())
    if vault_assets > 10:
        print(
